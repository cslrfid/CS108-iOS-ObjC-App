...
- (BOOL)readOEMData:(CSLBleInterface*)intf atAddr:(unsigned short)addr forData:(NSData*)data
{
    ...
    for (int i=0;i<COMMAND_TIMEOUT_5S;i++)  //receive data or time out in 5 seconds
        while((!([self.recvQueue count] != 0)) && (([[NSRunLoop currentRunLoop] runMode:NSDefaultRunLoopMode beforeDate:[NSDate dateWithTimeIntervalSinceNow:0.1]])));
    if ([self.recvQueue count] != 0)
        recvPacket = ((CSLBlePacket *)[self.recvQueue deqObject]);
    else
    {
    ...
    for (int i=0;i<COMMAND_TIMEOUT_5S;i++)  //receive data or time out in 5 seconds
        while((!([self.recvQueue count] >= 2)) && (([[NSRunLoop currentRunLoop] runMode:NSDefaultRunLoopMode beforeDate:[NSDate dateWithTimeIntervalSinceNow:0.1]])));
    if ([self.recvQueue count] >= 2)
        recvPacket = ((CSLBlePacket *)[self.recvQueue deqObject]);
    else
    {
        NSLog(@"Command timed out.");
    ...
    recvPacket=((CSLBlePacket *)[intf.recvQueue deqObject]);
...
- (BOOL)barcodeReader:(BOOL)enable
{
    ...
    for (int i=0;i<COMMAND_TIMEOUT_5S;i++)  //receive data or time out in 5 seconds
        while((!([self.recvQueue count] != 0)) && (([[NSRunLoop currentRunLoop] runMode:NSDefaultRunLoopMode beforeDate:[NSDate dateWithTimeIntervalSinceNow:0.1]])));
    if ([self.recvQueue count] != 0)
        payloadData = ((CSLBlePacket *)[self.recvQueue deqObject]).payload;
    else
    {
        connectStatus=CONNECTED;
        [self.delegate didInterfaceChangeConnectStatus:self]; //this will call the method for connections status chagnes.
...
- (BOOL)getBtFirmwareVersion:(NSString *)versionNumber
{
    ...
    for (int i=0;i<COMMAND_TIMEOUT_5S;i++)  //receive data or time out in 5 seconds
        while((!([self.recvQueue count] != 0)) && (([[NSRunLoop currentRunLoop] runMode:NSDefaultRunLoopMode beforeDate:[NSDate dateWithTimeIntervalSinceNow:0.1]])));
    if ([self.recvQueue count] != 0)
        versionInfo = ((CSLBlePacket *)[self.recvQueue deqObject]).payload;
    else
    {
        connectStatus=CONNECTED;
        [self.delegate didInterfaceChangeConnectStatus:self]; //this will call the method for connections status chagnes.
        NSLog(@"Command timed out.");
        NSLog(@"PGet BT IC firmware version: FAILED");
        return false;
    }
    connectStatus=CONNECTED;
    [self.delegate didInterfaceChangeConnectStatus:self]; //this will call the method for connections status chagnes.
...
- (BOOL) getConnectedDeviceName:(NSString *) deviceName
{
    ...
    if ([self.recvQueue count] != 0) {
        NSData * name = [((CSLBlePacket *)[self.recvQueue deqObject]).payload subdataWithRange:NSMakeRange(2, 21)];
...
- (BOOL)getSilLabIcVersion:(NSString *) slVersion
{
    ...
    if ([self.recvQueue count] != 0) {
        NSData * versionInfo = ((CSLBlePacket *)[self.recvQueue deqObject]).payload;
...
- (BOOL)getTriggerKeyStatus:(BOOL*) state {
    ...
    CSLBlePacket* packet= [[CSLBlePacket alloc] init];
    NSLog(@"----------------------------------------------------------------------");
    NSLog(@"Get trigger key state...");
    NSLog(@"----------------------------------------------------------------------");
    
    //Get trigger key state
    unsigned char TriggerKey[] = {0xA0, 0x01};
    packet.prefix=0xA7;
    packet.connection = Bluetooth;
    packet.payloadLength=0x02;
    ...
    packet.crc1=0;
    packet.crc2=0;
    packet.payload=[NSData dataWithBytes:TriggerKey length:sizeof(TriggerKey)];
    ...
    if ([self.recvQueue count] != 0) {
        NSData * keyState = ((CSLBlePacket *)[self.recvQueue deqObject]).payload;
        if (memcmp([keyState bytes], TriggerKey, 2) == 0)
        {
            *state = ((Byte *)[keyState bytes])[2] == 0x01;
            NSLog(@"Trigger key state: %@", *state ? @"ON" : @"OFF");
        }
    }
    else {
        connectStatus=CONNECTED;
        [self.delegate didInterfaceChangeConnectStatus:self]; //this will call the method for connections status chagnes.
        NSLog(@"Command timed out.");
        NSLog(@"Get trigger key state: FAILED");
        return false;
    }
    connectStatus=CONNECTED;
    [self.delegate didInterfaceChangeConnectStatus:self]; //this will call the method for connections status chagnes.
...
- (BOOL)getRfidBrdSerialNumber:(NSString*) serialNumber {
    ...
    if ([self.recvQueue count] != 0) {
        NSData * name = [((CSLBlePacket *)[self.recvQueue deqObject]).payload subdataWithRange:NSMakeRange(2, 16)];
...
- (BOOL)sendAbortCommand {
    ...
    if ([self.recvQueue count] >= 2) {
        recvPacket=((CSLBlePacket *)[self.recvQueue deqObject]);
        if (memcmp([recvPacket.payload bytes], abortCmd, 2) == 0 && ((Byte *)[recvPacket.payload bytes])[2] == 0x00)
            NSLog(@"RFID firmware command sent: OK");
        else {
            connectStatus=CONNECTED;
            [self.delegate didInterfaceChangeConnectStatus:self]; //this will call the method for connections status chagnes.
            NSLog(@"RFID firmware command sent: FAILED");
            return false;
        }
        
        recvPacket=((CSLBlePacket *)[self.recvQueue deqObject]);
        if ([[recvPacket getPacketPayloadInHexString] isEqualToString:@"81004003BFFCBFFCBFFC"])
...
-(BOOL)startInventory {
    ...
    NSLog(@"----------------------------------------------------------------------");
    NSLog(@"Please press the trigger button on the reader to start inventory....");
...
-(BOOL)stopInventory {
    ...
    if (connectStatus != CONNECTED)
    {
        NSLog(@"Abort command response failure.");
        return false;
    }
...
-(void)stopInventoryBlocking {
    ...
    if (connectStatus==BUSY && isRunningAsync)
    {
        NSLog(@"----------------------------------------------------------------------");
...
- (void)decodePacketsInBufferAsync;
{
    ...
    while (!(isAborted && ([self.recvQueue count] == 0)))
    {
        if ([self.recvQueue count] > 0)
        {
            ...
            else if ([[payload substringWithRange:NSMakeRange(0, 4)] isEqualToString:@"8100"])
            {
            ...
            else if ([[packet getPacketPayloadInHexString] isEqualToString:@"A103"]) {
                //send abort command
                NSLog(@"Trigger key being released");
                NSLog(@"Abort inventory...");
                unsigned char STOP_INV[] = {0x80, 0x02, 0x40, 0x03, 0x00, 0xF0, 0x0F, 0x00, 0x00, 0x00};
                packet.prefix=0xA7;
                packet.connection = Bluetooth;
                packet.payloadLength=0x0A;
                packet.deviceId=RFID;
                ...
                packet.crc1=0;
                packet.crc2=0;
                packet.payload=[NSData dataWithBytes:STOP_INV length:sizeof(STOP_INV)];
    ...
    NSLog(@"[decodePacketsInBufferAsync] Completed!");
}
